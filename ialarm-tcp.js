...
function MeianClient(host, port, uid, pwd) {
	 ...
	 */
	this._send = function (root, cmd, isList) {  //TODO list?
	...
	this._receive = function (buffer) {
		...
		if (data.Err && data.Err !== 'ERR|00') {
			...
			if (data.Root.Host) {
				...
				if (formatter) {
					response = formatter(data.Root.Host[cmdName]);
				}
			}
	...
	this.getLog = function () {
		...
		cmd['Err'] = null;
		//request
		self._send('/Root/Host/GetLog', cmd, true); //list?
	};
	...
	this.getOverlapZone = function () {
		...
		cmd['Err'] = null;
		//request
		self._send('/Root/Host/GetOverlapZone', cmd);  //list?
	};
