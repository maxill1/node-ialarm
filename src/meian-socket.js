...
var net = require('net');
var { MeianMessage, MeianMessageFunctions } = require('./meian-message');
...
function MeianSocket(host, port, uid, pwd) {
    ...
    self.executeCommand = function (commandNames, commandArgs) {
         ...
         */
        function disconnect() {
            console.log(`Disconnecting from ${host}:${port}`);
            if (socket) {
     ...
     */
            ...
            return new Promise((resolve, reject) => {
                try {
                    ...
                    if (data.Err && data.Err !== 'ERR|00') {
                        ...
                        var event = constants.events[cmdName] //custom command
        ...
        return new Promise((resolve, reject) => {
            ...
            const timer = setTimeout(() => {
                ...
                reject(new Error(`Meian socket timed out after ${socketTimeout} ms (your address/port may be wrong or the alarm may be hanging. Try connecting with your Android Phone, if not working either reboot the alarm and try again.)`));
            }, socketTimeout + 5000);
            ...
            socket.connect(port, host, function () {
                ...
                //Send the login request
                var loginMsg = MeianMessageFunctions.Client(uid, pwd);
                seq = loginMsg.seq;
